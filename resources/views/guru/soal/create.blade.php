    ...
    <div class="max-w-3xl mx-auto">
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
            <div class="p-5 border-b border-gray-100 bg-gray-50/50">
                ...
                <p class="text-[10px] text-gray-500 mt-0.5">Masukkan teks pertanyaan dan 4 pilihan jawaban (A-D)</p>
            </div>
            
            <form method="POST" action="{{ route('guru.soal.store', $bankSoal->id) }}" class="p-5 space-y-6">
                @csrf
                    ...
                    <div class="flex flex-wrap gap-3">
                        @foreach(['a', 'b', 'c', 'd'] as $key)
                            <label class="flex-1 min-w-[80px]">
                                <input type="radio" name="jawaban_benar" value="{{ $key }}" class="hidden peer" required {{ old('jawaban_benar') == $key ? 'checked' : '' }}>
