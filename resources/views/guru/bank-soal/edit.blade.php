    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">Edit Bank Soal</h1>

                    <form action="{{ route('guru.bank-soal.update', $bankSoal->id) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <select id="mata_pelajaran_id" name="mata_pelajaran_id" required 
                                ...
                                @foreach ($mataPelajarans as $mp)
                                    <option value="{{ $mp->id }}" {{ $bankSoal->mata_pelajaran_id == $mp->id ? 'selected' : '' }}>
                                        {{ $mp->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" id="nama_bank" name="nama_bank" value="{{ old('nama_bank', $bankSoal->nama_bank) }}" required
                        ...
                        <div>
                            ...
                            <input type="number" id="durasi" name="durasi" value="{{ old('durasi', $bankSoal->durasi) }}" required min="1" max="480"
