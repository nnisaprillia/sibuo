@extends('layouts.app', ['active' => 'siswa', 'title' => 'Manajemen Siswa'])
            ...
            <table class="w-full text-xs">
                <thead>
                    <tr class="border-b border-gray-200 bg-gray-50/50">
                        <th class="text-left px-5 py-3 text-gray-500 font-medium uppercase tracking-wider">Nama Siswa</th>
                        <th class="text-left px-5 py-3 text-gray-500 font-medium uppercase tracking-wider">Kelas / Jurusan</th>
                ...
                <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex flex-col">
                                    <span class="text-gray-900 font-medium">{{ $siswa->kelas->nama_kelas ?? '-' }}</span>
                                    <span class="text-[10px] text-gray-400">{{ $siswa->jurusan->nama_jurusan ?? '-' }}</span>
                                </div>
                            </td>
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form method="POST" action="{{ route('admin.siswa.destroy', $siswa) }}" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus" onclick="return confirm('Apakah Anda yakin ingin menghapus siswa ini?')">
                    ...
                    @empty
                        <tr>
                            <td colspan="4">
                                <x-empty-state entity="siswa" :route="route('admin.siswa.create')" />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
