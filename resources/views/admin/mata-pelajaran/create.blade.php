@extends('layouts.app', ['active' => 'mata-pelajaran', 'title' => 'Tambah Mata Pelajaran'])

@section('breadcrumb')
    <div class="flex items-center gap-2 text-[10px] text-gray-400 uppercase tracking-widest font-semibold">
        <a href="{{ route('admin.mata-pelajaran.index') }}" class="hover:text-blue-500 transition-colors">Mata Pelajaran</a>
    ...
    <div class="max-w-xl mx-auto">
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
            <div class="p-5 border-b border-gray-100 bg-gray-50/50">
                <h3 class="text-sm font-medium text-gray-900">Data Mata Pelajaran</h3>
                <p class="text-[10px] text-gray-500 mt-0.5">Definisikan mata pelajaran baru yang akan diajarkan</p>
            </div>
            
            <form method="POST" action="{{ route('admin.mata-pelajaran.store') }}" class="p-5 space-y-4">
                @csrf
                ...
                <div class="space-y-1">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="3"
                        ...
                        placeholder="Berikan ringkasan singkat mengenai mata pelajaran ini...">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi')
                        <p class="text-[10px] text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
