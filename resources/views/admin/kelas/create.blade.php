@extends('layouts.app', ['active' => 'kelas', 'title' => 'Tambah Kelas'])
    ...
    <div class="max-w-xl mx-auto">
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
            <div class="p-5 border-b border-gray-100 bg-gray-50/50">
                <h3 class="text-sm font-medium text-gray-900">Data Kelas</h3>
                <p class="text-[10px] text-gray-500 mt-0.5">Daftarkan rombongan belajar atau kelas baru</p>
            </div>
            
            <form method="POST" action="{{ route('admin.kelas.store') }}" class="p-5 space-y-4">
                @csrf
                ...
                <div class="space-y-1">
                    ...
                    <select name="tingkat" id="tingkat" required
                        ...
                        <option value="SMA" {{ old('tingkat') === 'SMA' ? 'selected' : '' }}>SMA/SMK (Menengah Atas)</option>
                    </select>
                    @error('tingkat')
                        <p class="text-[10px] text-red-600 mt-1">{{ $message }}</p>
                    @enderror
